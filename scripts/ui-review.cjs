...
let prescriptionBody,firstPrescription;let prescriptionAttempts=0;
...
(async()=>{
 ...
 await page.addInitScript(()=>{sessionStorage.setItem('quremed-token','eyJhbGciOiJIUzI1NiJ9.'+btoa(JSON.stringify({sub:'demo-user'}))+'.synthetic');localStorage.setItem('quremed.appearance.v1','eco')});
 ...
 else if(p==='/operations')data={version:'4.0.0',metrics:[{label:'Вільних ліжок',value:12,path:'/rooms'},{label:'Надходжень сьогодні',value:3,path:'/patients'},{label:'Прострочених завдань',value:2,path:'/tasks'},{label:'В роботі',value:8,path:'/tasks'}],staff:[{id:'d1',name:'Мельник Олександр',role:'DOCTOR'},{id:'n1',name:'Петренко Наталія',role:'NURSE'},{id:'t1',name:'Кравчук Андрій',role:'THERAPIST'}],appointments:[]};
 ...
 else if(p==='/tasks'&&route.request().method()==='POST'){prescriptionBody=route.request().postDataJSON();prescriptionAttempts++;if(prescriptionAttempts===1){firstPrescription=prescriptionBody;await route.abort('failed');return}data={id:'saved-demo'}}
 ...
 await page.getByRole('button',{name:'Призначити лікування',exact:true}).click();const prescription=page.locator('dialog').filter({has:page.getByRole('heading',{name:'Нове призначення'})}).last();await prescription.waitFor();assert.ok(await prescription.getByText(patient.name,{exact:true}).isVisible());await prescription.getByLabel('Назва процедури',{exact:true}).fill('Демонстраційна процедура');await prescription.getByRole('button',{name:'Підписати призначення'}).click();await prescription.getByRole('button',{name:'Повторити збереження без дублювання'}).waitFor();await page.reload();await page.getByRole('button',{name:/Коваль Олена/}).click();await page.getByRole('button',{name:'Призначити лікування',exact:true}).click();const restored=page.locator('dialog').filter({has:page.getByRole('heading',{name:'Нове призначення'})}).last();await restored.getByRole('button',{name:'Повторити збереження без дублювання'}).click();await page.waitForTimeout(300);assert.deepEqual(prescriptionBody,firstPrescription);assert.equal(prescriptionAttempts,2);assert.equal(prescriptionBody?.patient_id,id);await page.keyboard.press('Escape');
 ...
 if(process.env.GH_TOKEN){for(const file of fs.readdirSync('ui-review').filter(f=>f.endsWith('.png'))){const payload=JSON.stringify({encoding:'utf-8',content:fs.readFileSync('ui-review/'+file).toString('base64')});const result=JSON.parse(execFileSync('gh',['api','--method','POST',`repos/${process.env.GITHUB_REPOSITORY}/git/blobs`,'--input','-'],{input:payload,encoding:'utf8'}));console.log('UI_IMAGE '+JSON.stringify({file,sha:result.sha}));}}
 }finally{server.kill()}
