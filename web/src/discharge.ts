type ClinicalEntry={kind:string;body:string;data?:Record<string,unknown>;created_at?:string;author?:string};
...
const multi=(value:unknown)=>esc(value).replace(/\r?\n/g,'<br>');
const dateOnly=(value:unknown)=>value?new Date(String(value)).toLocaleDateString('uk-UA'):'—';
const dateTime=(value:unknown)=>value?new Date(String(value)).toLocaleString('uk-UA'):'—';
...
function section(title:string,body:string){return `<section class="rf-ds-section"><h2>${esc(title)}</h2>${body}</section>`;}
function field(label:string,value:unknown){return `<div class="rf-ds-field"><b>${esc(label)}:</b> <span>${multi(value)}</span></div>`;}

function buildSheet(data:DischargePayload){
...
async function printDischarge(button:HTMLButtonElement){
  ...
  try{
    ...
    const host=document.createElement('div');host.innerHTML=buildSheet(data as DischargePayload);const sheet=host.firstElementChild;if(!sheet)throw new Error('Не вдалося підготувати документ');document.body.appendChild(sheet);
...
function enhance(){
  installStyles();
  document.querySelectorAll<HTMLDialogElement>('dialog.care-dialog[open]').forEach(dialog=>{
    ...
    dialog.querySelectorAll<HTMLElement>('section.panel.padded h3').forEach(title=>{
      if(title.textContent?.trim()!=='Виписний підсумок')return;
      title.closest('section')?.querySelectorAll<HTMLElement>('.field > span').forEach(label=>{
