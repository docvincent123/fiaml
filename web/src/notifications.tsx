...
export function DeviceSettings(){
 const [info]=useState<any>(()=>{try{return JSON.parse(android()?.deviceInfo?.()||'null')}catch{return null}});
 if(!android())return null;
 return <section className="panel padded"><h2>Цей пристрій</h2>{info?.id?<><p>Модель: {info.model}</p><p>Android: {info.android}</p><p>Версія APK: {info.version}</p><p>Пакет: {info.package}</p><p>Код інсталяції:</p><code style={{overflowWrap:'anywhere'}}>{info.id}</code><p className="muted">Код зберігається при оновленні цього застосунку. Видалення програми або очищення даних створить новий код. Це не IMEI.</p></>:<p>Оновіть APK для перегляду моделі та постійного коду.</p>}</section>;
}
