...
export function DischargeReport({patient:p}:{patient:any}){
 ...
 const assessments=entries.filter((e:any)=>e.kind==='ASSESSMENT');
 ...
 <p><b>Пацієнт № {p.patient_number}:</b> {p.name} · <b>Дата народження:</b> {date(p.birth_date)}</p>
 ...
 <section><h2>При надходженні</h2><p><b>Направлення:</b> {ad.referral||'Не внесено'}</p><p><b>Скарги:</b> {ad.complaints||assessments.at(-1)?.data?.complaints||summary?.data?.complaints||'Не внесено'}</p><p><b>Первинний діагноз:</b> {assessments.at(-1)?.data?.diagnosis||'Не внесено'}</p></section><section><h2>Огляди лікаря</h2>{assessments.length?assessments.slice().reverse().map((e:any)=><div key={e.id}><p><b>{date(e.created_at)} · {e.author}</b></p><p>{e.body}</p><p>{e.data?.diagnosis}</p><p>{e.data?.plan}</p>{entries.filter((c:any)=>c.corrects_id===e.id).map((c:any)=><p key={c.id}><b>Виправлення:</b> {c.body} · {c.author}</p>)}</div>):<p>Огляди не внесено.</p>}</section><section><h2>Діагноз при виписці</h2><p>{summary?.data?.diagnosis||'Не внесено у виписний підсумок'}</p></section>
 <section><h2>Перебіг і результат лікування</h2><p>{summary?.data?.treatment_summary||summary?.body||'Підсумок не внесено'}</p></section>
 <section><h2>Фактично виконані призначення ({completed.length})</h2>{completed.length?<table><thead><tr><th>Дата</th><th>Лікування / процедура</th><th>Виконавець і результат</th></tr></thead><tbody>{completed.map((t:any)=><tr key={t.id}><td>{date(t.completed_at)}</td><td>{t.task_type}<p>{t.description}</p>{t.medication&&<p>{t.medication} · {t.dose} {t.dose_unit} · {t.route}</p>}</td><td>{t.executor||'—'}<p>{t.outcome||'Результат окремо не описано'}</p></td></tr>)}</tbody></table>:<p>Підтверджених виконань у цій госпіталізації немає.</p>}</section>
 ...
 <footer><p>Підсумок склав: {summary?.author||'—'} · {time(summary?.created_at)}</p><div className="signature-grid"><p>Лікуючий лікар: {ad.doctor_name||summary?.author||'________________'}<br/>Підпис: ____________________</p><p>Завідувач: {summary?.data?.head_name||'____________________'}<br/>Підпис: ____________________</p></div><p>Дата підписання: ____________________</p><small>Patients · № {p.patient_number}</small></footer>
 </article>;
 return <><label className="field">Період лікування<select value={admissionId} onChange={e=>setAdmissionId(e.target.value)}>{p.admissions.map((a:any)=><option key={a.id} value={a.id}>{date(a.admitted_at)} — {a.discharged_at?date(a.discharged_at):'триває'}</option>)}</select></label><p className="muted">Виписний підсумок лікар заповнює у «Медичні записи → Виписний підсумок». До проведеного лікування входять лише підтверджені виконання. У Windows кнопка PDF зберігає виписку без IP. Для звичайного друку вимкніть «Колонтитули» у вікні друку.</p><button className="primary no-print" onClick={()=>{const native=(window as any).QureMedAndroid;const bridge=(window as any).chrome?.webview;if(bridge)bridge.postMessage({type:"quremed.discharge.pdf"});else if(native)native.print();else window.print();}}>Друк / зберегти PDF виписки</button><button className="ghost no-print" onClick={()=>{const bridge=(window as any).chrome?.webview;if(bridge)bridge.postMessage({type:"quremed.discharge.pdf"});else window.print();}}>PDF для Windows без адреси сервера</button>{document}{createPortal(<div className="discharge-print">{document}</div>,window.document.body)}</>;
}
