...
before(async()=>{
 ...
 else{const pg=new PGlite();for(const file of ['001_initial.sql','002_care.sql','003_room_retirement.sql','004_staff_workflow.sql','005_patient_numbers.sql','006_request_receipts.sql','007_delivery_health.sql'])await pg.exec(readFileSync(new URL('../sql/'+file,import.meta.url),'utf8'));let queue=Promise.resolve();db={query:(q:string,v:any[]=[])=>pg.query(q,v),tx:async(fn:any)=>{let release!:()=>void;const before=queue;queue=new Promise<void>(r=>release=r);await before;try{return await pg.transaction(tx=>fn({query:(q:string,v:any[]=[])=>tx.query(q,v)}))}finally{release()}}} as any;close=()=>pg.close();}
