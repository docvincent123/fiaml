...
before(async()=>{
 ...
 else {const pg=new PGlite();for(const file of ['001_initial.sql','003_room_retirement.sql','004_staff_workflow.sql','005_patient_numbers.sql'])await pg.exec(readFileSync(new URL('../sql/'+file,import.meta.url),'utf8'));
 ...
 patient=await registerTest(admin,{name:'Тестовий Пацієнт',bed_id:bed.id,doctor_id:doctor.id});cabinet=await c.cabinet(admin,{name:'Масаж тест',type:'massage'});
});
...
test('Bed uniqueness survives competing registrations',async()=>{await assert.rejects(()=>registerTest(admin,{name:'Другий',bed_id:bed.id}));const count=(await db.query('SELECT count(*)::int AS n FROM patients WHERE name=$1',['Другий'])).rows[0].n;assert.equal(count,0);});
test('Cabinet concurrent bookings reject overlap and allow adjacency',async()=>{const p2=await registerTest(admin,{name:'Інший'});const slot={cabinet_id:cabinet.id,starts_at:'2030-01-01T10:00:00Z',ends_at:'2030-01-01T11:00:00Z'};const r=await Promise.allSettled([c.appointment(admin,{...slot,patient_id:patient.id}),c.appointment(admin,{...slot,patient_id:p2.id})]);assert.equal(r.filter(x=>x.status==='fulfilled').length,1);await c.appointment(admin,{...slot,patient_id:patient.id,starts_at:'2030-01-01T11:00:00Z',ends_at:'2030-01-01T12:00:00Z'});});
