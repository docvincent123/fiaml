...
export class Clinic {
 ...
 async register(a:Actor,input:any){allow(a,'patients.manage');const b=z.object({name:text,birth_date:z.string().date().nullable().optional(),phone:z.string().max(50).default(''),bed_id:optionalId,doctor_id:optionalId}).parse(input);
  return this.db.tx(async c=>{await this.validateDoctor(c,b.doctor_id);const p=(await c.query('INSERT INTO patients(name,birth_date,phone) VALUES($1,$2,$3) RETURNING *',[b.name,b.birth_date??null,b.phone])).rows[0];
 ...
 async rooms(a:Actor){allow(a,'rooms.manage');return (await this.db.query('SELECT r.id,r.room_number,COALESCE(jsonb_agg(jsonb_build_object(\'id\',b.id,\'bed_number\',b.bed_number,\'qr_uid\',b.qr_uid,\'patient_id\',p.id,\'patient_name\',p.name) ORDER BY b.bed_number) FILTER(WHERE b.id IS NOT NULL),\'[]\') AS beds FROM rooms r LEFT JOIN beds b ON b.room_id=r.id LEFT JOIN admissions a ON a.bed_id=b.id AND a.discharged_at IS NULL LEFT JOIN patients p ON p.id=a.patient_id GROUP BY r.id ORDER BY r.room_number')).rows;}
 ...
 async appointmentStatus(a:Actor,id:string,input:any){allow(a,'cabinets.manage');uuid.parse(id);const b=z.object({status:z.enum(['CANCELLED','COMPLETED'])}).parse(input);return this.db.tx(async c=>{const r=(await c.query("UPDATE appointments SET status=$1 WHERE id=$2 AND status='BOOKED' RETURNING id",[b.status,id])).rows[0];if(!r) throw new ConflictException('Запис вже закрито');await this.audit(c,a,'appointment.'+b.status.toLowerCase(),id);await this.changed(c);return r;});}
